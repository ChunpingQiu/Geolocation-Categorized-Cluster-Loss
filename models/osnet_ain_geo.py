...
__all__ = [
    'osnet_ain_x1_0', 'osnet_ain_x0_75_geo', 'osnet_ain_x0_5', 'osnet_ain_x0_25_geo'
]

pretrained_urls = {
    'osnet_ain_x1_0':
    'https://erniudataset.cdn.bcebos.com/osnet_ain_x1_0_imagenet.pth',
    'osnet_ain_x0_75':
    'https://erniudataset.cdn.bcebos.com/osnet_ain_x0_75_imagenet.pyth',
    'osnet_ain_x0_5':
    'https://drive.google.com/uc?id=1KusKvEYyKGDTUBVRxRiz55G31wkihB6l',
    'osnet_ain_x0_25':
    'https://erniudataset.cdn.bcebos.com/osnet_ain_x0_25_imagenet.pyth'
}
...
class OSNet(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.fc = self._construct_fc_layer(
            self.feature_dim, channels[3], dropout_p=drop_rate
        )
        # identity classification layer
        if self.loss == 'softmax':
            self.classifier = nn.Linear(self.feature_dim, num_classes)
        elif self.loss == 'amsoftmax':
            self.classifier=AMSoftmaxLayer(self.feature_dim, num_classes)
        elif self.loss == 'cluster':
            self.classifier = nn.Linear(self.feature_dim, num_classes)
        self._init_params()
    ...
    def forward(self, x1, x2=None):
        y1,v1 = self._forward(x1)
        if x2 is not None:
            y2,v2= self._forward(x2)
            return y1, y2, v1, v2
        else:
            return  y1,v1
    def _forward(self, x, return_featuremaps=False):
...
def init_pretrained_weights(model, key=''):
    ...
    try:
        ...
        if e.errno == errno.EEXIST:
            ...
            raise
    filename = key + '_imagenet.pyth'
    cached_file = os.path.join(model_dir, filename)
    # print("downloading:",pretrained_urls[key])
...
##########
def osnet_ain_x1_0(
    num_classes=701, pretrained=True, loss='softmax', **kwargs
):
    model = OSNet(
        num_classes,
        blocks=[
...
if __name__ == '__main__':
    model=osnet_ain_x0_75_geo(class_num=701,pretrained=False,loss="softmax", drop_rate=0.75)
    inp=torch.ones((8,3,256,256))
    ou=model(inp,inp,inp,inp)
    print(ou)